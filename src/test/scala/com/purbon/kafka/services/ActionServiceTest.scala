...
class ActionServiceTest  extends FunSpec
  ...
  describe ("An Action service manager") {
    ...
    it("execute the migration task if the migrate action if received") {
      val config = new Config(action = Some("migrate"))
